...
function initMusicSystem() {
    ...
    if (musicSystem.audio) {
        ...
        musicSystem.audio.addEventListener('canplay', () => {
            ...
            // Müzik yüklendiğinde otomatik başlat
            if (gameState.soundEnabled) {
                playMusic();
            }
        });
        ...
        // Sayfa etkileşimi olmadan müziği başlatmaya çalış
        setTimeout(() => {
            if (gameState.soundEnabled) {
                playMusic();
            }
...
window.addEventListener('load', () => {
    ...
    const startMusic = () => {
        if (gameState.soundEnabled && !musicSystem.isPlaying) {
            playMusic();
        }
