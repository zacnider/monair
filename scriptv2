...
const CONFIG = {
    ...
    performance: {
        ...
        maxVisibleMonanimals: 50,   // Limit visible monanimals for performance
        maxParticles: 20,           // Limit maximum particles
        optimizeRendering: true,    // Use optimized rendering techniques
        maxTotalMonanimals: 50,     // Maximum total monanimal count
        cleanupInterval: 5000,      // More frequent cleanup
        maxBalloons: 3              // Maximum balloons at once
    }
};
    ...
    update() {
        ...
        if (this.state === 'launching' || this.state === 'flying') {
            if (CONFIG.balloons.type === 'zeppelin') {
                ...
                // Balloon flies upward (original behavior)
                this.y = lerp(this.y, this.targetY, CONFIG.balloons.flySpeed * 0.008);
                this.x += windEffect;
...
function update() {
    // Update balloons with culling for offscreen objects
    gameState.balloons.forEach(balloon => {
...
function draw() {
    ...
    visibleMonanimals.forEach(person => person.draw());
...
function togglePause() {
    // Reset game instead of just pausing/resuming
    resetGame();
    console.log('🔄 Game reset due to pause toggle');
}
