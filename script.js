...
// Monanimal Character Data - Each image represents a specific transaction type
const MONANIMAL_DATA = {
    images: [
        'images/Butterfly2.png',      // transfer
        'images/cfbde5.png',          // approve
        'images/Component_23_4.png',  // swap
        'images/IMG_7966.PNG.png',    // stake
        'images/monad_ikan.png',      // unstake
        'images/adacv.png',           // claim
        'images/Molandak.png'         // contract
    ],
    // Mapping of transaction types to image indices
    txTypeToImageIndex: {
        'transfer': 0,   // Butterfly2.png
        'approve': 1,    // cfbde5.png
        'swap': 2,       // Component_23_4.png
        'stake': 3,      // IMG_7966.PNG.png
        'unstake': 4,    // monad_ikan.png
        'claim': 5,      // adacv.png
        'contract': 6    // Molandak.png
    },
    size: {
        width: 24,
        height: 24
...
class Balloon {
    ...
    isClicked(mouseX, mouseY) {
        ...
        // Balon tıklama alanını da büyüttük
        return distance < this.size + 50;
    }
}
...
class MonanimalPerson {
    constructor(x, y, txData = null) {
        ...
        // Set image index based on transaction type
        this.imageIndex = MONANIMAL_DATA.txTypeToImageIndex[this.txType] || 0;
    }
    ...
    isClicked(mouseX, mouseY) {
        const distance = Math.sqrt((mouseX - this.x) ** 2 + (mouseY - this.y) ** 2);
        // Tıklama alanını daha da büyüttük - çok daha kolay tıklanabilir
        return distance < this.size * 4;
    }
}
...
function initMusicSystem() {
    ...
    if (musicSystem.audio) {
        ...
        musicSystem.audio.addEventListener('canplay', () => {
            ...
            // Müzik yüklendiğinde otomatik başlatmayı dene
            if (gameState.soundEnabled) {
                setTimeout(() => {
                    playMusic();
...
function initGame() {
    ...
    // Müziği otomatik başlatmayı dene (sayfa yüklendikten sonra)
    setTimeout(() => {
        if (gameState.soundEnabled && musicSystem.audio) {
            console.log('🎵 Sayfa yüklendi, müziği otomatik başlatmaya çalışıyor...');
            playMusic();
        }
    }, 1000);
...
function createNewBalloon(blockData) {
    ...
    if (transactions.length > 0) {
        ...
        for (let i = 0; i < txCount; i++) {
            ...
            // Calculate spawn position near the balloon
            const spawnRadius = 80 + Math.random() * 40; // 80-120 pixel radius around balloon
            const spawnAngle = (i / txCount) * Math.PI * 2; // Distribute evenly around balloon
            const spawnX = balloon.x + Math.cos(spawnAngle) * spawnRadius;
            const spawnY = balloon.y + Math.sin(spawnAngle) * spawnRadius;
            
            // Ensure spawn position is within canvas bounds and on ground level
            const finalSpawnX = Math.max(50, Math.min(CONFIG.canvas.width - 50, spawnX));
            const finalSpawnY = Math.max(CONFIG.canvas.height - 150, Math.min(CONFIG.canvas.height - 100, spawnY));
            
            // Create a new monanimal with the confirmed transaction data near the balloon
            const monanimal = new MonanimalPerson(
                finalSpawnX,
                finalSpawnY,
            ...
            // Immediately target this balloon and set to walking state
            ...
            console.log(`🐠 Monanimal spawned near balloon at (${finalSpawnX.toFixed(1)}, ${finalSpawnY.toFixed(1)}) for ${tx.txType} transaction`);
        }
    }
...
canvas.addEventListener('click', (event) => {
    ...
    // Canvas boyutlarına göre koordinatları ölçekle
    ...
    // Önce tüm tıklanabilir monanimal'ları bul - daha geniş arama alanı
    ...
    if (!balloonClicked && monanimalsClicked.length > 0) {
        ...
        // Eğer hiçbir şey tıklanmadıysa, en yakın monanimal'ı bul (daha geniş arama)
        let closestMonanimal = null;
        let minDistance = Infinity;
        
        for (const monanimal of gameState.people) {
            if (monanimal.state !== 'flying') {
                const distance = Math.sqrt((mouseX - monanimal.x) ** 2 + (mouseY - monanimal.y) ** 2);
                if (distance < 150 && distance < minDistance) { // 150px içindeki en yakın
                    minDistance = distance;
                    closestMonanimal = monanimal;
                }
            }
        }
        ...
        if (closestMonanimal) {
            showMonanimalTxInfo(closestMonanimal);
            console.log(`🐠 En yakın monanimal tıklandı: ${closestMonanimal.txType} (mesafe: ${minDistance.toFixed(1)})`);
        } else {
            // Debug: Tıklama pozisyonunu logla
            console.log(`🖱️ Tıklama pozisyonu: (${mouseX.toFixed(1)}, ${mouseY.toFixed(1)}) - Hiçbir şey bulunamadı`);
        }
    }
});
...
canvas.addEventListener('mousemove', (event) => {
    ...
    // Eğer hiçbir şeyin üzerinde değilse, yakında bir şey var mı kontrol et
    if (!isHovering) {
        for (const monanimal of gameState.people) {
            if (monanimal.state !== 'flying') {
                const distance = Math.sqrt((mouseX - monanimal.x) ** 2 + (mouseY - monanimal.y) ** 2);
                if (distance < 100) { // 100px yakınlık
                    isHovering = true;
                    break;
                }
            }
...
function showMonanimalTxInfo(monanimal) {
    ...
    // Get Monanimal image path
    const monanimalsImagePath = MONANIMAL_DATA.images[monanimal.imageIndex];
    const monanimalsImageName = monanimalsImagePath.split('/').pop(); // Get filename only
    
    content.innerHTML = `
        <div class="tx-item">
            <div style="display: flex; align-items: center; margin-bottom: 15px;">
                <img src="${monanimalsImagePath}" alt="${txInfo.name} Monanimal"
                     style="width: 48px; height: 48px; margin-right: 15px; border-radius: 8px;
                            border: 2px solid ${txInfo.color}; background: white; padding: 4px;"
                ...
                <div>
                    ...
                    <br>
                    <small style="color: #666;">Monanimal: ${monanimalsImageName}</small>
                </div>
            </div>
...
function showBlockTxPanel(blockHeight) {
    ...
    Object.keys(txGroups).forEach(txType => {
        ...
        // Get Monanimal image for this transaction type
        const imageIndex = MONANIMAL_DATA.txTypeToImageIndex[txType] || 0;
        const monanimalsImagePath = MONANIMAL_DATA.images[imageIndex];
        const monanimalsImageName = monanimalsImagePath.split('/').pop(); // Get filename only
        
        txListHTML += `
            <div style="margin-bottom: 15px;">
                <h4 style="color: ${txInfo.color}; margin: 10px 0 5px 0; display: flex; align-items: center;">
                    <img src="${monanimalsImagePath}" alt="${txInfo.name} Monanimal"
                         style="width: 32px; height: 32px; margin-right: 10px; border-radius: 6px;
                                border: 2px solid ${txInfo.color}; background: white; padding: 2px;"
                         onerror="this.style.display='none'">
                    <span>${txInfo.emoji} ${txInfo.name} (${groupTxs.length})</span>
                    <small style="margin-left: 10px; color: #666; font-weight: normal; font-size: 11px;">
                        ${monanimalsImageName}
                    </small>
                </h4>
        `;
