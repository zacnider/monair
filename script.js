...
const gameState = {
    ...
    clouds: [],
    particles: [],
    fireCharacters: [], // Fire characters for battle system
...
// Load fire character for battle
const FIRE_CHARACTER = new Image();
FIRE_CHARACTER.onload = () => {
    console.log('✅ Fire character loaded successfully');
};
FIRE_CHARACTER.onerror = () => {
    console.error('❌ Failed to load fire character');
};
FIRE_CHARACTER.src = 'images/fire.png';
...
class Balloon {
    ...
    drawPassengerInfo() {
        ...
        // Savunucu çizimi (eğer aktif savunucu varsa)
        if (this.activeDefender && this.activeDefender.hasWeapon) {
            this.drawDefender();
        }
    }
    ...
    drawDefender() {
        ...
        // Savunucu silahı pozisyonu (sepetin üstünde)
        let defenderX, defenderY;
        
        if (CONFIG.balloons.type === 'zeppelin') {
            // Zeppelin için gondola üstü
            defenderX = this.x;
            defenderY = this.y + this.size * 0.5 + 15; // Gondola üstü
        } else {
            // Balon için sepet üstü
            const basketY = this.y + this.size * 0.55 + 35;
            defenderX = this.x;
            defenderY = basketY - 5; // Sepet üstü
        }
        
        // Savunucu silahı (sepetin üstünde)
        if (this.activeDefender.defenderTarget) {
            const dx = this.activeDefender.defenderTarget.x - defenderX;
            const dy = this.activeDefender.defenderTarget.y - defenderY;
            ...
            // Ateş efekti (eğer savunuyorsa)
            if (this.activeDefender.isDefending &&
                Date.now() - this.activeDefender.defendStartTime < 400) {
            ...
            // Silah parıltısı
            if (this.activeDefender.isDefender) {
...
class MonanimalPerson {
    ...
    draw() {
        ...
        // Silah çizimi (eğer saldırıyorsa)
        if (this.hasWeapon || this.isAttacking) {
            this.drawWeapon();
        }
    ...
    drawWeapon() {
        ...
        // Silah yönünü belirle
        let weaponAngle = 0;
        if (this.battleTarget) {
            const dx = this.battleTarget.x - this.x;
            const dy = this.battleTarget.y - this.y;
        ...
        // Silah boyutu (savunucu için daha küçük)
        const weaponScale = this.isDefender ? 0.7 : 1.0;
        const weaponLength = 25 * weaponScale;
        const weaponWidth = 4 * weaponScale;
        
        // Silah gölgesi
        ctx.strokeStyle = 'rgba(0, 0, 0, 0.4)';
        ctx.lineWidth = weaponWidth + 1;
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(this.size + 1, 1);
        ctx.lineTo(this.size + weaponLength + 1, 1);
        ctx.stroke();
        
        // Ana silah gövdesi (gerçekçi tüfek)
        ctx.strokeStyle = '#2F2F2F';
        ctx.lineWidth = weaponWidth;
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(this.size, 0);
        ctx.lineTo(this.size + weaponLength, 0);
        ctx.stroke();
        
        // Silah kabzası (gerçekçi)
        ctx.fillStyle = '#654321';
        const gripWidth = 10 * weaponScale;
        const gripHeight = 6 * weaponScale;
        ctx.fillRect(this.size - 6 * weaponScale, -gripHeight/2, gripWidth, gripHeight);
        
        // Kabza detayları
        ctx.strokeStyle = '#4A2C17';
        ctx.lineWidth = 1;
        for (let i = 0; i < 3; i++) {
            const lineX = this.size - 4 * weaponScale + (i * 2 * weaponScale);
            ctx.beginPath();
            ctx.moveTo(lineX, -gripHeight/2);
            ctx.lineTo(lineX, gripHeight/2);
            ctx.stroke();
        }
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(this.size, -1 * weaponScale);
        ctx.lineTo(this.size + weaponLength - 3, -1 * weaponScale);
        ctx.moveTo(this.size, 1 * weaponScale);
        ctx.lineTo(this.size + weaponLength - 3, 1 * weaponScale);
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.arc(this.size + weaponLength, 0, 2 * weaponScale, 0, Math.PI * 2);
        ctx.fill();
        
        // İç namlu deliği
        ctx.fillStyle = '#000000';
        ctx.beginPath();
        ctx.arc(this.size + weaponLength, 0, 1 * weaponScale, 0, Math.PI * 2);
        ctx.fill();
        
        // Tetik koruyucu
        ctx.strokeStyle = '#2F2F2F';
        ctx.lineWidth = 2 * weaponScale;
        ctx.beginPath();
        ctx.arc(this.size - 2 * weaponScale, 2 * weaponScale, 3 * weaponScale, 0, Math.PI);
        ctx.stroke();
        
        // Ateş efekti (eğer saldırıyorsa)
        if ((this.isAttacking || this.isDefending) &&
            Date.now() - (this.attackStartTime || this.defendStartTime) < 400) {
            
            const flashSize = 8 * weaponScale;
            
            // Büyük alev
            ctx.fillStyle = '#FFD700';
            ctx.beginPath();
            ctx.arc(this.size + weaponLength + 5, 0, flashSize, 0, Math.PI * 2);
            ctx.fill();
            
            // Orta alev
            ctx.fillStyle = '#FF6B35';
            ctx.beginPath();
            ctx.arc(this.size + weaponLength + 5, 0, flashSize * 0.7, 0, Math.PI * 2);
            ctx.fill();
            
            // İç alev
            ctx.fillStyle = '#FF4500';
            ctx.beginPath();
            ctx.arc(this.size + weaponLength + 5, 0, flashSize * 0.4, 0, Math.PI * 2);
            ctx.fill();
            
            // Kıvılcımlar
            for (let i = 0; i < 6; i++) {
                const sparkX = this.size + weaponLength + 5 + Math.random() * 12 - 6;
                const sparkY = Math.random() * 12 - 6;
                ctx.fillStyle = Math.random() > 0.5 ? '#FFD700' : '#FF6B35';
                ctx.beginPath();
        ...
        // Savaş modunda silah parıltısı
        if (this.isInBattle || this.isDefender) {
            ctx.shadowColor = this.isDefender ? '#FF6B35' : '#FFD700';
            ctx.shadowBlur = 6;
            ctx.strokeStyle = this.isDefender ? '#FF6B35' : '#FFD700';
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(this.size - 6 * weaponScale, -gripHeight/2 - 1);
            ctx.lineTo(this.size + weaponLength, -2 * weaponScale);
            ctx.lineTo(this.size + weaponLength, 2 * weaponScale);
            ctx.lineTo(this.size - 6 * weaponScale, gripHeight/2 + 1);
            ctx.closePath();
            ctx.stroke();
            ctx.shadowBlur = 0;
        }
...
// Fire Character Class for Battle System
class FireCharacter {
    constructor(x, y) {
        this.x = x;
        this.y = y;
        this.size = 96; // Fire character size
        this.lastShotTime = Date.now();
        this.battleStartTime = Date.now();
        this.isActive = true;
        this.animation = 0;
        this.id = Math.random().toString(36).substr(2, 6);
        this.spawnAnimationPhase = 0; // Spawn animation phase
        this.isSpawning = true; // Spawn state
        this.spawnDuration = 2000; // 2 seconds spawn animation
        this.battleDuration = 10000; // 10 seconds battle duration
        this.targetBalloons = []; // All flying balloons as targets
        this.lightningEffects = []; // Lightning effects during spawn
        
        // Create lightning effects during spawn
        this.createSpawnLightning();
        
        // Find all flying balloons as targets
        this.findTargetBalloons();
        
        // Setup defenders for all target balloons
        this.setupDefenders();
        
        console.log(`🔥 Fire character spawned! Targeting ${this.targetBalloons.length} balloons`);
    }
    
    createSpawnLightning() {
        // Create multiple lightning strikes around spawn point
        for (let i = 0; i < 8; i++) {
            setTimeout(() => {
            ...
            }, i * 200); // Lightning strikes every 200ms
        }
    ...
    playThunderSound() {
        try {
            ...
            console.log('Audio not supported for thunder sound');
        }
    }
    
    findTargetBalloons() {
        // Find all flying balloons
        this.targetBalloons = gameState.balloons.filter(balloon =>
            (balloon.state === 'flying' || balloon.state === 'launching') &&
            balloon.passengers.length > 0 &&
            balloon.opacity > 0
        );
    }
    
    setupDefenders() {
        // Setup defenders for each target balloon
        this.targetBalloons.forEach(balloon => {
            if (balloon.passengers.length > 0) {
                const defender = balloon.passengers[Math.floor(Math.random() * balloon.passengers.length)];
                defender.person.isDefender = true;
                defender.person.hasWeapon = true;
                defender.person.defenderTarget = this;
                balloon.activeDefender = defender.person;
            }
        });
    }
    ...
    update() {
        ...
        if (this.isSpawning) {
            this.spawnAnimationPhase += 0.05;
            
            // End spawn animation after duration
            if (Date.now() - this.battleStartTime > this.spawnDuration) {
                this.isSpawning = false;
                console.log('🔥 Fire character spawn complete - battle begins!');
            }
            return; // Don't do anything else during spawn
        }
        
        // Check battle duration (10 seconds max)
        if (Date.now() - this.battleStartTime > this.battleDuration) {
            console.log('🔥 Battle duration exceeded - fire character retreating');
            this.isActive = false;
            return;
        }
        
        // Update target balloons list
        this.findTargetBalloons();
        
        // Check if no balloons left in air
        if (this.targetBalloons.length === 0) {
            console.log('🔥 No balloons left in air - fire character retreating');
            this.isActive = false;
            return;
        }
        
        // Fire at all balloons every 800ms
        if (Date.now() - this.lastShotTime > 800) {
            this.fireAtAllBalloons();
            this.lastShotTime = Date.now();
        }
    }
    
    fireAtAllBalloons() {
        // Fire at each target balloon
        this.targetBalloons.forEach((balloon, index) => {
            setTimeout(() => {
                this.fireShotAtBalloon(balloon);
                
                // Counter fire from balloon defender
                if (balloon.activeDefender && Math.random() < 0.8) {
                    setTimeout(() => {
                        this.fireCounterShotFromBalloon(balloon);
                    }, 300 + Math.random() * 400); // Random delay for realism
                }
            }, index * 100); // Stagger shots to different balloons
        });
    }
    
    fireShotAtBalloon(balloon) {
        if (!balloon || balloon.state === 'gone' || balloon.opacity <= 0) return;
        
        // Create muzzle flash
        this.createMuzzleFlash(this.x, this.y);
        
        // Create bullet trail to specific balloon
        this.createBulletTrail(this.x, this.y, balloon.x, balloon.y);
    ...
    fireCounterShotFromBalloon(balloon) {
        if (!balloon.activeDefender || balloon.state === 'gone') return;
        
        // Counter fire from balloon
        this.createMuzzleFlash(balloon.x, balloon.y - 20);
        this.createBulletTrail(balloon.x, balloon.y - 20, this.x, this.y);
        ...
        // Defender animation
        balloon.activeDefender.isDefending = true;
        balloon.activeDefender.defendStartTime = Date.now();
        
        setTimeout(() => {
            if (balloon.activeDefender) {
                balloon.activeDefender.isDefending = false;
            }
        }, 400);
    }
    ...
    fireShot() {
        ...
        // Create bullet trail
        this.createBulletTrail(this.x, this.y, this.targetBalloon.x, this.targetBalloon.y);
    ...
    fireCounterShot() {
        if (!this.targetBalloon.activeDefender) return;
        
        // Counter fire from balloon
        this.createMuzzleFlash(this.targetBalloon.x, this.targetBalloon.y - 20);
        this.createBulletTrail(this.targetBalloon.x, this.targetBalloon.y - 20, this.x, this.y);
        ...
        // Defender animation
        this.targetBalloon.activeDefender.isDefending = true;
        this.targetBalloon.activeDefender.defendStartTime = Date.now();
        
        setTimeout(() => {
            if (this.targetBalloon.activeDefender) {
                this.targetBalloon.activeDefender.isDefending = false;
            }
        }, 400);
    }
    ...
    createBulletTrail(fromX, fromY, toX, toY) {
    ...
    playGunSound() {
        try {
            ...
            console.log('Audio not supported for gun sound');
        }
    }
    ...
    draw() {
        ...
        if (this.isSpawning) {
            this.drawSpawnAnimation();
        }
        
        // Calculate opacity and effects based on state
        let opacity = 1;
        let glowIntensity = 1;
        
        if (this.isSpawning) {
            opacity = Math.min(1, this.spawnAnimationPhase * 2);
            glowIntensity = 2 + Math.sin(this.spawnAnimationPhase * 10) * 0.5;
        }
        ...
        // Draw fire character image
        if (FIRE_CHARACTER && FIRE_CHARACTER.complete && FIRE_CHARACTER.naturalWidth > 0) {
            // Add bounce animation
            const bounce = Math.sin(this.animation * 4) * 3;
            const spawnScale = this.isSpawning ? 0.8 + (this.spawnAnimationPhase * 0.2) : 1;
            
            const drawSize = this.size * spawnScale;
            
            ctx.drawImage(
                FIRE_CHARACTER,
                this.x - drawSize/2,
                this.y - drawSize/2 + bounce,
                drawSize,
                drawSize
            );
        } else {
            // Fallback fire character with enhanced effects
            const spawnScale = this.isSpawning ? 0.8 + (this.spawnAnimationPhase * 0.2) : 1;
            const drawSize = (this.size/2) * spawnScale;
            ...
            ctx.shadowBlur = 0;
            ctx.fillStyle = '#FFD700';
            ctx.font = `${drawSize * 1.5}px Arial`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
    ...
    drawSpawnAnimation() {
        // Lightning effects around spawn point
        for (let i = 0; i < 6; i++) {
            const angle = (i / 6) * Math.PI * 2 + this.spawnAnimationPhase * 2;
            const radius = 60 + Math.sin(this.spawnAnimationPhase * 8) * 20;
            const x = this.x + Math.cos(angle) * radius;
            const y = this.y + Math.sin(angle) * radius;
            
            ctx.fillStyle = `rgba(255, 255, 0, ${0.8 - this.spawnAnimationPhase})`;
            ctx.beginPath();
            ctx.arc(x, y, 3, 0, Math.PI * 2);
            ctx.fill();
            
            // Lightning bolts
            ctx.strokeStyle = `rgba(255, 255, 255, ${0.9 - this.spawnAnimationPhase})`;
            ctx.lineWidth = 2;
            ctx.beginPath();
        ...
        // Energy circle
        ctx.strokeStyle = `rgba(255, 215, 0, ${1 - this.spawnAnimationPhase})`;
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.arc(this.x, this.y, 80 * this.spawnAnimationPhase, 0, Math.PI * 2);
        ctx.stroke();
    }
    
    drawBattleEffects() {
        // Battle aura
        const battleIntensity = Math.sin(this.animation * 6) * 0.3 + 0.7;
        
        ctx.strokeStyle = `rgba(255, 69, 0, ${battleIntensity})`;
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(this.x, this.y, this.size/2 + 10, 0, Math.PI * 2);
        ctx.stroke();
        
        // Fire particles around character
        for (let i = 0; i < 3; i++) {
            const angle = this.animation * 2 + (i * Math.PI * 2 / 3);
            const radius = 40 + Math.sin(this.animation * 4 + i) * 10;
            const x = this.x + Math.cos(angle) * radius;
            const y = this.y + Math.sin(angle) * radius;
    ...
    drawWeapon() {
        if (!this.targetBalloon) return;
        
        ctx.save();
        ctx.translate(this.x, this.y);
        
        // Calculate weapon angle towards target
        const dx = this.targetBalloon.x - this.x;
        const dy = this.targetBalloon.y - this.y;
        const weaponAngle = Math.atan2(dy, dx);
        ...
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(this.size/2 + 1, 1);
        ctx.lineTo(this.size/2 + weaponLength + 1, 1);
        ctx.stroke();
        ...
        ctx.lineTo(this.size/2 + weaponLength, 0);
        ctx.stroke();
        ...
        const gripWidth = 10;
        const gripHeight = 6;
        ctx.fillRect(this.size/2 - 6, -gripHeight/2, gripWidth, gripHeight);
        
        // Muzzle
        ctx.fillStyle = '#0F0F0F';
        ctx.beginPath();
        ctx.arc(this.size/2 + weaponLength, 0, 2, 0, Math.PI * 2);
        ctx.fill();
        
        // Muzzle flash effect
        if (this.isAttacking && Date.now() - this.attackStartTime < 400) {
            const flashSize = 8;
            
            ctx.fillStyle = '#FFD700';
            ctx.beginPath();
            ctx.arc(this.size/2 + weaponLength + 5, 0, flashSize, 0, Math.PI * 2);
            ctx.fill();
            
            ctx.fillStyle = '#FF6B35';
            ctx.beginPath();
            ctx.arc(this.size/2 + weaponLength + 5, 0, flashSize * 0.7, 0, Math.PI * 2);
            ctx.fill();
            
            ctx.fillStyle = '#FF4500';
            ctx.beginPath();
            ctx.arc(this.size/2 + weaponLength + 5, 0, flashSize * 0.4, 0, Math.PI * 2);
            ctx.fill();
        }
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(this.size/2 - 6, -gripHeight/2 - 1);
        ctx.lineTo(this.size/2 + weaponLength, -2);
        ctx.lineTo(this.size/2 + weaponLength, 2);
        ctx.lineTo(this.size/2 - 6, gripHeight/2 + 1);
        ctx.closePath();
        ctx.stroke();
        ctx.shadowBlur = 0;
...
function update() {
    ...
    // Fire character battle system - balona binemeyen monanimals'lar için
    updateBattleEvents();
}

// Fire character battle system
function updateBattleEvents() {
    // Her 3 saniyede bir savaş olayı kontrolü
    if (!gameState.lastBattleCheck) gameState.lastBattleCheck = Date.now();
    
    if (Date.now() - gameState.lastBattleCheck > 3000) { // 3 saniye
        gameState.lastBattleCheck = Date.now();
        
        // %25 şans ile fire character savaş olayı başlat
        if (Math.random() < 0.25) {
            startBattleEvent();
        }
    }
    
    // Aktif fire character savaşlarını güncelle
    updateActiveBattles();
}

function startBattleEvent() {
    // Zaten aktif fire character var mı kontrol et (sadece 1 tane olabilir)
    if (gameState.fireCharacters.length > 0) {
        return; // Sadece 1 fire character aktif olabilir
    }
    
    // Uçan balonları bul
    const flyingBalloons = gameState.balloons.filter(b =>
        (b.state === 'flying' || b.state === 'launching') && b.passengers.length > 0
    );
    
    if (flyingBalloons.length === 0) {
        return; // Hedef balon yok
    }
    
    // Fire character oluştur (tüm balonları hedef alacak)
    const fireCharacter = new FireCharacter(
        100 + Math.random() * 200, // X pozisyonu
        CONFIG.canvas.height - 150 + Math.random() * 50 // Y pozisyonu
    );
    
    gameState.fireCharacters.push(fireCharacter);
    
    // Battle message göster
    showBattleMessage(fireCharacter, null);
    
    console.log(`🔥 Fire character battle started! Targeting ${flyingBalloons.length} balloons for 10 seconds`);
}

function updateActiveBattles() {
    // Fire characters'ları güncelle
    gameState.fireCharacters.forEach((fireCharacter, index) => {
        fireCharacter.update();
        
        // Fire character artık aktif değilse kaldır
        if (!fireCharacter.isActive) {
            // Savunucuyu temizle
            if (fireCharacter.targetBalloon && fireCharacter.targetBalloon.activeDefender) {
                const defender = fireCharacter.targetBalloon.activeDefender;
                defender.isDefender = false;
                defender.hasWeapon = false;
                defender.defenderTarget = null;
                defender.isDefending = false;
                fireCharacter.targetBalloon.activeDefender = null;
            }
            
            gameState.fireCharacters.splice(index, 1);
            console.log('Fire character removed - target balloon destroyed');
        }
    });
}

// Fire character battle functions are now handled in the FireCharacter class
...
function createBulletTrail(fromX, fromY, toX, toY) {
    ...
    for (let i = 0; i < steps; i++) {
        ...
        }, i * 50); // Her 50ms'de bir parçacık
    }
}

function createBattleEffects(attacker, targetBalloon) {
    // Savaş alanında duman efektleri
    for (let i = 0; i < 15; i++) {
        setTimeout(() => {
            const x = attacker.x + Math.random() * 200 - 100;
            const y = attacker.y + Math.random() * 100 - 50;
    ...
    showBattleMessage(attacker, targetBalloon);
}

function showBattleMessage(fireCharacter, targetBalloon) {
    // Fire character battle mesajları (İngilizce)
    const messages = [
        `⚡ Lightning strikes! Fire character emerges!`,
        `🔥 Epic battle begins - Fire vs All Balloons!`,
        `💥 Thunder roars! 10-second showdown starts!`,
        `⚔️ Fire character attacks all flying balloons!`,
        `🌩️ Lightning spawns angry fire warrior!`
    ];
...
function playGunSound() {
    try {
        ...
        // Silah sesi için frekans ayarları
        oscillator.frequency.setValueAtTime(200, audioContext.currentTime);
        oscillator.frequency.exponentialRampToValueAtTime(50, audioContext.currentTime + 0.1);
...
function draw() {
    ...
    // Draw fire characters
    gameState.fireCharacters.forEach((fireCharacter, index) => {
        try {
            fireCharacter.draw();
        } catch (error) {
            console.warn(`Fire character ${index} çizim hatası:`, error);
            // Hatalı fire character'ı kaldır
            gameState.fireCharacters.splice(index, 1);
        }
    });
...
function drawDebugInfo() {
    ...
    ctx.fillRect(10, CONFIG.canvas.height - 220, 300, 200);
    ...
    const debugInfo = [
        ...
        `Fire Characters: ${gameState.fireCharacters.length}`,
        ...
        `In Battle: ${gameState.people.filter(p => p.isInBattle).length}`,
        `Defenders: ${gameState.people.filter(p => p.isDefender).length}`
    ];
    
    debugInfo.forEach((info, i) => {
        ctx.fillText(info, 20, CONFIG.canvas.height - 200 + i * 15);
    });
    
    // Debug: Fire character tıklama alanlarını göster
    ctx.strokeStyle = 'rgba(255, 165, 0, 0.7)';
    ctx.lineWidth = 3;
    gameState.fireCharacters.forEach(fireChar => {
        ctx.beginPath();
        ctx.arc(fireChar.x, fireChar.y, fireChar.size, 0, Math.PI * 2);
        ctx.stroke();
    });
...
function resetGame() {
    ...
    gameState.people = [];
    gameState.particles = [];
    gameState.fireCharacters = []; // Fire characters'ları da temizle
